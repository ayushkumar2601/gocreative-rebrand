...
export default function PortfolioShowcase() {
  ...
  return (
    <section id="portfolio" className="w-full bg-[#080D1A] text-white py-24 sm:py-32 relative overflow-hidden border-t border-white/10 select-none">
                    ...
                    >
                      ...
                      <div className="absolute inset-0 z-10 bg-black overflow-hidden rounded-[34px] sm:rounded-[40px]">
                        <video
                          ...
                          loop
                          playsInline
                          muted={!isCenter || isAudioMuted}
                          className="w-full h-full object-cover absolute inset-0"
                        />
                      </div>

                      {/* Top Bar inside Video Screen */}
                      <div className="relative z-30 flex justify-between items-center pt-2 px-3 pointer-events-auto">
                        <div className="flex items-center gap-2">
                          <div className="w-9 h-9 rounded-full bg-black/70 backdrop-blur-md border border-white/30 flex items-center justify-center text-sm shadow">
                            {card.icon}
                          </div>
                          <span className="bg-black/70 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-black text-[#00B4FF] border border-[#00B4FF]/40 uppercase shadow">
                            {card.badgeText}
                          </span>
                        </div>

                        {/* Audio Status Pill (Central phone shows Audio On) */}
                        {isCenter && (
                          <button
                            onClick={() => setIsAudioMuted(!isAudioMuted)}
                            className="bg-black/80 backdrop-blur-md px-3 py-1 rounded-full border border-[#FF1493] text-white text-[10px] font-black flex items-center gap-1.5 shadow-lg hover:scale-105 transition-transform cursor-pointer"
                          >
                            <span>{!isAudioMuted ? "🔊" : "🔇"}</span>
                            <span>{!isAudioMuted ? "AUDIO ON" : "MUTED"}</span>
                          </button>
                        )}
                        {!isCenter && (
                          <div className="bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-full text-white/60 text-[9px] font-bold border border-white/10 flex items-center gap-1">
                            <span>🔇</span>
                            <span>Muted</span>
                          </div>
                        )}
